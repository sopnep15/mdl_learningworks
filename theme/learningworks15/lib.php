 ...
 * the {@link theme_learningworks15_less_variables()} here because we need to create selectors or
 * alter existing ones.
 *
...
function theme_learningworks15_extra_less($theme)
    {
        $content = '';
 ...
 */
function theme_learningworks15_less_variables($theme)
    {
        $variables = array();
        ...
        if (!empty($theme->settings->barcolor)) {
            $variables['BarColor'] = $theme->settings->barcolor;
        }
        if (!empty($theme->settings->bartextcolor)) {
            $variables['BarTextColor'] = $theme->settings->bartextcolor;
        }
        if (!empty($theme->settings->barlinkcolor)) {
            $variables['BarLinkColor'] = $theme->settings->barlinkcolor;
        }

        if (!empty($theme->settings->btnprimarycolor)) {
            $variables['btnPrimaryColor'] = $theme->settings->btnprimarycolor;
        }
        if (!empty($theme->settings->btnoverridecolor)) {
            $variables['btnOverrideColor'] = $theme->settings->btnoverridecolor;
        }
        if (!empty($theme->settings->btntextcolor)) {
            $variables['btnTextColor'] = $theme->settings->btntextcolor;
        }
        if (!empty($theme->settings->btntextcolor)) {
            $variables['blockHeaderBackground'] = $theme->settings->blockheaderbackground;
        }
        if (!empty($theme->settings->btntextcolor)) {
            $variables['blockHeaderText'] = $theme->settings->blockheadertext;
        }
 ...
 */
function theme_learningworks15_process_css($css, $theme) {

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_learningworks15_set_logo($css, $logo);
    ...
    $css = theme_learningworks15_set_customcss($css, $customcss);
 ...
 */
function theme_learningworks15_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
...
function theme_learningworks15_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
 ...
 */
function theme_learningworks15_set_customcss($css, $customcss) {
 ...
 * @return null
 */
function theme_learningworks15_page_init(moodle_page $page) {
