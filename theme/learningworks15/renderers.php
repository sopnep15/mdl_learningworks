...
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.
     ...
     */
    class theme_learningworks15_mod_quiz_renderer extends mod_quiz_renderer
    {
        ...
        public function review_link($url, $reviewinpopup, $popupoptions) {
            if ($reviewinpopup) {
                ...
                $reviewButton = "<a title='Review your responses to this attempt' href='".$url."'><div class='btn'>".get_string('review', 'quiz')."</div></a>";
     ...
     * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
     */
    class theme_learningworks15_mod_assign_renderer extends plugin_renderer_base {
        ...
        public function render_assign_submission_status(assign_submission_status $status) {
            //OVERRIDE HERE TO SHOW WHO HAS MARKED THE SUBMISSION
            global $DB;
            $grader = $DB->get_record('assign_grades',array('assignment'=>$status->submission->assignment,'userid'=>$status->submission->userid));
            if($grader) {
                $user = user_get_users_by_id(array($grader->grader));
                $edittime = $grader->timemodified;
            }
            ...
            if ($user) {
                ...
                $row = new html_table_row();
                $cell1 = new html_table_cell(get_string('gradedby', 'assign'));
                $userdescription = array_values($user)[0]->firstname .' '. array_values($user)[0]->lastname;
                $cell2 = new html_table_cell($userdescription);
                $row->cells = array($cell1, $cell2);
                $t->data[] = $row;
            }
            ...
            if ($edittime) {
                ...
                $row = new html_table_row();
                $cell1 = new html_table_cell(get_string('gradedon', 'assign'));
                $cell2 = new html_table_cell(userdate($edittime));
                $row->cells = array($cell1, $cell2);
                $t->data[] = $row;
            }
